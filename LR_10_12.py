...
# データ読み込み
hrv_df = pd.read_csv("extracted_data_10_12.csv")
...
matched_df.to_csv("extract_data_10_12_matached.csv", index=False)
print("✅ Saved Match data: extract_data_10_12_matached.csv")
print('-' * 100)
...
# CSV読み込み
hrv_df = pd.read_csv("extract_data_10_12_matached.csv")
icd_df = pd.read_csv("20241206_010627mimic-iii-wave-match-BasicTable-icd.csv")
...
# === 1. ファイル読み込み ===
file_path = "extracted_data_10_12.csv"
...
# === 5. 保存 ===
output_path = "hrv_features_window_avg_10_12.csv"
...
# === データ読み込み（すでに患者ごとに1行） ===
hrv_df = pd.read_csv("hrv_features_window_avg_10_12.csv")
icd_df = pd.read_csv("extract_data_10_12_matached.csv")
...
for col in selected_features:
    ...
    summary_data.append({
        'Feature': col,
        'P-value': p_value,
        'Survived(199)': f"{mean_alive:.4f} ± {std_alive:.4f}",
        'non-Survived(31)': f"{mean_dead:.4f} ± {std_dead:.4f}"
    })
...
print('-' * 100)
print(summary_df)
summary_df.to_csv("selected_feature_summary_10_12.csv", index=False)  # 保存したい場合
print('-' * 100)
...
df_avg.to_csv("all_threshold_metrics_0.001_10_12.csv", index=False, encoding='utf-8-sig')
