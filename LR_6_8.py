...
# データ読み込み
hrv_df = pd.read_csv("extracted_data_6_8.csv")
...
matched_df.to_csv("extract_data_6_8_matached.csv", index=False)
print("✅ Saved Match data: extract_data_6_8_matached.csv")
print('-' * 100)
...
# CSV読み込み
hrv_df = pd.read_csv("extract_data_6_8_matached.csv")
icd_df = pd.read_csv("20241206_010627mimic-iii-wave-match-BasicTable-icd.csv")
...
# === 1. ファイル読み込み ===
file_path = "extracted_data_6_8.csv"
...
# === 5. 保存 ===
output_path = "hrv_features_window_avg_6_8.csv"
...
# === データ読み込み（すでに患者ごとに1行） ===
hrv_df = pd.read_csv("hrv_features_window_avg_6_8.csv")
icd_df = pd.read_csv("extract_data_6_8_matached.csv")
...
selected_features = pval_df[pval_df['p_value'] < 0.001]['feature'].tolist()
# === 抜く ===
exclude = [
    'window_size',
    'rrIntv_quantile50',
...
for col in selected_features:
    ...
    summary_data.append({
        'Feature': col,
        'P-value': p_value,
        'Survived(208)': f"{mean_alive:.4f} ± {std_alive:.4f}",
...
print('-' * 100)
print(summary_df)
summary_df.to_csv("selected_feature_summary_6_8.csv", index=False)  # 保存したい場合
print('-' * 100)
...
df_avg.to_csv("all_threshold_metrics_0.001_6_8.csv", index=False, encoding='utf-8-sig')
